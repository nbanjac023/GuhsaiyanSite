...
@section('content')
    ...
    <section class="shipping u-margin-top-xxxl" id="cartToggleArea">
        ...
        <div class="container">
            ...
            <form action="{{ route('address.update', auth()->user()->address) }}" method="POST" class="shipping__form col-11 col-sm-11 col-md-8 col-lg-6">
                @method('PATCH')
                @csrf
                ...
                <div class="shipping__form-group">
                    ...
                    <input type="text" name="street_name" id="streetName" class="shipping__form-input" autocomplete="street-address" value="{{ $address->street_name }}" required>
                    @error('street_name')
                ...
                <div class="shipping__form-group">
                    ...
                    <input type="text" name="apt_number" id="aptNumber" class="shipping__form-input" value="{{ $address->apt_number }}" required>
                    @error('apt_number')
                        <p class="shipping__form-error">{{ $message }}</p>
                    @enderror
                </div>
                <div class="shipping__form-group">
                    ...
                    <input type="text" name="city" id="city" class="shipping__form-input" value="{{ $address->city }}" required>
                    @error('city')
                        <p class="shipping__form-error">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="shipping__form-group u-w-40">
                        ...
                        <select class="shipping__form-select" name="country" id="country" autocomplete="country-name" required>
                            @foreach($countries as $country)
                                <option value="{{ $country->name }}" {{ $address->country == $country->name ? 'selected' : '' }}>{{ $country->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="shipping__form-group u-w-40">
                        <label for="postalCode" class="shipping__form-label">Poštanski broj</label>
                        <input type="number" name="postal_code" id="postalCode" class="shipping__form-input" value="{{ $address->postal_code }}" autocomplete="postal_code" required>
                    </div>
                    @error('postal_code')
                        <p class="shipping__form-error">{{ $message }}</p>
                    @enderror
                ...
                <div class="shipping__form-group">
                    ...
                    <input type="text" name="phone_number" id="phoneNumber" class="shipping__form-input" value="{{ auth()->user()->phone_number }}" autocomplete="tel" required>
                    @error('phone_number')
                        <p class="shipping__form-error">{{ $message }}</p>
                    @enderror
                </div>
